...
import { getComingMovies } from "../../apis/getMovies";
import MovieCard from "../../Components/MovieCard";
import { setUpcomingMovies } from "../../store/actions/movieActions";
...
const UpcomingMovie = () => {
    ...
    const popularMovies = useSelector((state) => {
        return state.movies.upcoming_movies;
    });
    ...
    useEffect(() => {
        const fetchAPI = async () => {
            const res = await getComingMovies(page);
            if (res?.status === 200) {
                setTotalPage(res.data.total_pages);
                dispatch(
                    setUpcomingMovies(popularMovies.concat(res?.data?.results))
                );
            }
        };
        fetchAPI();
